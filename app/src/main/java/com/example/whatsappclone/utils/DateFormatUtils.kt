...
fun Date.formatAsTime() : String{
    val hour = calendar.get(HOUR_OF_DAY).toString().padEnd(2,'0')
    val minute = calendar.get(MINUTE).toString().padStart(2,'0')
    return "$hour:$minute"
}

fun Date.formateAsYesterday(c:Context) : String{
...
fun Date.formatAsFull(context: Context, abbreviated: Boolean = false): String{
    val month = if (abbreviated) "LLL" else "LLL"
...
fun Date.formatAsListItem(context: Context): String{
    ...
    return when{
        ...
        isYesterday() -> formateAsYesterday(context)
